                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="basic-datatables" class="display table table-striped table-hover" >
                                    ...
                                    <tbody>
                                        @foreach ($po as $e=>$item)
                                        <tr>
                                            <td>{{ $e+1 }}</td>
                                            <td>{{ $item->kode }}</td>
                                            <td>{{ $item->customer->nama }}</td>
                                            <td style="text-align: end;">{{ number_format($item->grand_total,0) }}</td>
                                            <td class="text-center">
                                                ...
                                                <a href="#" data-id="{{ $item->id }}"
                                                    ...
                                                    <a class="swal-confirm btn btn-sm btn-danger btn-shadow mt-2 mb-2" data-id="{{ $item->id }}" href="#">DELETE
                                                        <form action="{{ route('hapus-po', $item->id) }}"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">TAMBAH DATA PO</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
                <form class="form form-vertical" method="post" enctype="multipart/form-data"
                    action="{{ route('save-po') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="kode" @error('kode') class="text-danger" @enderror>KODE TRANSAKSI PO @error('kode')
                                        | {{ $message }}
                                        @enderror</label>
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="nama" @error('nama') class="text-danger" @enderror>CUSTOMER @error('nama')
                                        | {{ $message }}
                                        @enderror</label>
